...
- 蓝色表达导航、当前对象、主要操作、可跳转操作以及仍在等待完整数据的状态；绿色只表达成功、可用和明确符合；黄色表达警告、部分符合或接近目标；红色表达真实危险、失败或明确不符合。共享 `Primary` 使用操作蓝，不使用成功绿；成功徽标和符合结果不得伪装成按钮。推荐 Roll 等只读对照中的红色只能用于低强度状态点、结果文字和淡色底板，不能复用危险按钮表面，也不能暗示系统要求分解。紫色只用于 AI；珊瑚红时间强调色只表达动态倒计时，并与错误红使用不同 token 和色值。
