...
- Offer 卡片显示：`账号已有 N 件`；数量为零时显示：`账号未拥有`。
...
- 删除 T45 推荐状态对 T46 的隐式依赖，明确旧 `recommended` 字段迁移策略。
...
- 在 `packages/ui/src/vendors/` 卡片中增加短拥有状态和“只看未拥有”筛选。
- 保持现有成本、购买条件、加载、空、失败和 Xur 定时器行为不变。

交付物：拥有数量卡片、未拥有筛选、部分账号数据状态。
...
- 账号中没有实例时显示“账号未拥有”，而不是因数据未加载显示同样文案。
