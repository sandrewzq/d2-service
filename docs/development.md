...
- 武器推荐遵循“四人工来源严格 CSV + DIM 原生 Wishlist → SQLite 查询 → DTO 传输”的边界。31 列 CSV 只允许 Aegis、LGpig、YXCRALLXY、Sayalarry；DIM `.txt / .wishlist` 由主进程独立解析，不转换为普通六栏 CSV。人工来源的武器与推荐项只接受官方 Hash 或规范化后的 Bungie 官方全名完全相同，禁止 `contains`、唯一包含、简称、同义词和大师属性词干归类。导入由 Desktop 完成预览、Manifest 语义校验、内容指纹复核和单事务替换；缺少当前语义校验版本的旧人工来源库为 `unverified`，可保留但不得参与留拆判断。DIM 在线更新固定到准确 commit SHA，网络或激活失败时继续使用上一版。Renderer 不直接读取 CSV 或 SQLite；第三方再分发许可未确认前公开安装包不得内置人工来源 CSV。v0.0.22 已发布基线见 [T20 完成摘要](work/backlog/T20-weapon-recommendation-vault-cleanup.md)，下一版本的发布身份扩展与性能依赖统一见 [T22](work/backlog/T22-renderer-event-driven-performance.md)。
- 仓库推荐在 Renderer 运行时只维护 `Map<instanceId, RecommendationCardSummary>`；同名版本共享推荐但每个实例使用自身实际 Roll 独立计算。人工来源六项优先级为 `Perk 1 / Perk 2 > 第一列 / 第二列 > 大师 / 起源特性`，内部固定保留“全部符合 / 核心符合 / 接近推荐 / 关键缺失 / 未符合 / 仅推荐这把武器 / 数据不完整”七种状态；玩家摘要先显示实际要求的核心 `Perk x/y`，再显示全部明确要求栏位的 `完整 x/y`，无法核对项单独标记待核对。同栏候选为任选其一、不同栏分别核对。多个来源不得累计分数，同一用途下正反结论冲突或存在无法核对时首层归入“需要比较”；`general` 与 PVE、PVP 均重叠。DIM 保持无序且全部必需的原生 Hash 组合，在仓库卡与装备详情中固定排在人工来源和个人推荐之后，并使用“符合 n 套 · 最佳组合 x/y”的独立组合摘要。匹配结果按 Roll 指纹、Manifest 版本、人工知识库 revision、DIM revision、玩家来源覆盖 revision 和算法版本缓存；位置、光等、锁定和玩家标签不使 Roll 分析失效。卡片、推荐筛选、账号摘要、清理保护和同名整理首屏复用轻量实例摘要，禁止在单卡渲染中遍历完整 Wishlist；逐栏候选、图标、说明、链接和 DIM 规则只在详情、玩家选中的当前同名来源对比或显式验收报告中按需读取，不得作为全账号 Map 常驻 Renderer。完整可掉落池只在详情中由玩家展开后按 Manifest 版本加载，实例完整 Roll 只在玩家展开后按 `instanceId + rollFingerprint` 读取和复用。批量操作必须跳过锁定、精确配装引用、手动保留、独特 Roll、来源冲突、数据不完整和未覆盖武器，应用永不自动锁定、解锁或分解。
...
- 仓库筛选使用菜单私有 `VaultQueryIndex`：按实例维护物品范围、锁定、槽位、位置、弹药、类型、稀有度、阶级、职业、伤害、套装和框架的 ID Set。单件 Patch 只更新变化记录，结果和分面先做 Set 交集，再对缩小后的候选执行搜索、标签、愿望单、个人目标和护甲阈值等原有精确规则；不得为结果、槽位、位置、框架和类型数量分别重复扫描整账号装备。
